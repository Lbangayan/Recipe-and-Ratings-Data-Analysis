...
In our assessment of the data we had only three columns with any missing data. The first was average rating which we know to be missing by design since it is a product of the way we merged our data. If a recipe didn’t have a review then its average rating would be missing since there is nothing to base it off of. Then we looked at the name but it had only one missing value so we ignored it. That left only the ‘description’ column to look at. At first, we actually believed ‘description’ would be NMAR. The reason why is because ‘description’ is a column where sometimes users are to not contribute a description if the description for the recipe is too long. However, some additional data we want to bring in to test this is ‘n_ingredients’, because the more ingredients a recipe has, the more complex the recipe is, so a description couldn’t summarize the recipe well. Some other data we want to bring in to test if ‘description’ is NMAR is calories; what if for really high-calorie foods, the contributors don’t submit the description as seeing connotations of the food being high-calorie in the description could make users averse to making the recipe?
